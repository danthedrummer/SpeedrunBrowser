...
class MainActivity : AppCompatActivity() {
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    return when (item.itemId) {
      R.id.action_show_favourites -> {
        binding.mainToolbar.title = resources.getText(R.string.favourites)
        displayFavourites()
        true
      }

      R.id.action_clear_all -> {
        binding.mainToolbar.title = resources.getText(R.string.games_title)
  ...
  private fun displayFavourites() {
    ...
    if (favourites != null && favourites.list.isNotEmpty()) {
